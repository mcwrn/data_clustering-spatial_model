...
expected_data=read.csv("expected_counts.csv", header=T, sep=",", encoding = "UTF-8")
observed_data=read.csv("respiratory_admissions.csv", header=T, sep=",", encoding = "UTF-8")

expected_data$Id=expected_data$X.U.FEFF.code
observed_data$Id=observed_data$X.U.FEFF.IG
...
Convergence of the chain and comment on them. Plot the fitted values on two maps and comment on them.
```{r}
...
```{r}
summary(cars)
setwd("C:/Users/macie/OneDrive - Wydział Nauk Ekonomicznych Uniwersytet Warszawski/zima 23-24/a spatial/labs/MACIEJ/Labs (2)_basicMBCslides/Labs")
########################################
...
```{r}
plot(1, type = "n", xlab = "Number of Clusters", ylab = "BIC", main = "")
# Plot the lines
